...
cdef class SLIM_BPR_Cython_Epoch:
    ...
    def epochIteration_Cython(self):
        ...
        cdef long i, j
        cdef long index, seen_item, n_current_batch
        ...
        for n_current_batch in range(n_total_batch):
            ...
            if self.train_with_sparse_weights and n_current_batch % (n_total_batch/5) == 0 and n_current_batch!=0:
                self.S_sparse.rebalance_tree(TopK=self.topK)


            if self.verbose and ((n_current_batch+1) % print_step==0 or n_current_batch==n_total_batch-1):
                print("Processed {} ({:4.1f}%) in {:.2f} seconds. BPR loss is {:.2E}. Sample per second: {:.0f}".format(
                    (n_current_batch+1)*self.batch_size,
                    100.0* float((n_current_batch+1)*self.batch_size)/self.n_users,
                    time.time() - start_time_batch,
                    loss/((n_current_batch+1)*self.batch_size + 1),
                    float((n_current_batch+1)*self.batch_size + 1) / (time.time() - start_time_epoch)))
