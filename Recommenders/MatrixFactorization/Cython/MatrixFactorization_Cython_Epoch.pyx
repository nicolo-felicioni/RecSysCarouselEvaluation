...
cdef class MatrixFactorization_Cython_Epoch:
    ...
    def epochIteration_Cython_FUNK_SVD_SGD(self):
        ...
        cdef long factor_index, n_current_batch, n_sample_in_batch, processed_samples_last_print, print_block_size = 500
        ...
        for n_current_batch in range(n_total_batch):
            ...
            if self.verbose and (processed_samples_last_print >= print_block_size or n_current_batch == n_total_batch-1):
                ...
                samples_per_sec = n_current_batch/(time.time() - start_time_epoch)

                print_block_size = int(samples_per_sec*30)

                if current_time - last_print_time > 30 or n_current_batch == n_total_batch-1:

                    print("{}: Processed {} ({:4.1f}%) in {:.2f} seconds. MSE loss {:.2E}. Sample per second: {:.0f}".format(
                        self.algorithm_name,
    ...
    def epochIteration_Cython_ASY_SVD_SGD(self):
        ...
        cdef MSE_sample sample
        cdef long n_current_batch, n_sample_in_batch, processed_samples_last_print, print_block_size = 500
        cdef double prediction, prediction_error
        cdef double local_gradient_item, local_gradient_user, local_gradient_bias_item, local_gradient_bias_user, local_gradient_bias_global
