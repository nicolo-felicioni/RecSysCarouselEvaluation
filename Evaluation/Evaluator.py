...
from Evaluation.metrics import roc_auc, precision, precision_recall_min_denominator, recall, MAP, MRR, ndcg, arhr, \
...
class Evaluator(object):
    ...
    def __init__(self, URM_test_list, cutoff_list, min_ratings_per_user=1, exclude_seen=True,
        ...
        if not np.all(users_to_evaluate_mask):
            self._print("Ignoring {} ({:.2f}%) Users that have less than {} test interactions".format(np.sum(np.logical_not(users_to_evaluate_mask)),
    ...
    def _compute_metrics_on_recommendation_list(self, test_user_batch_array, recommended_items_batch_list, scores_batch, results_dict):
        ...
        for batch_user_index in range(len(recommended_items_batch_list)):
            ...
            for cutoff in self.cutoff_list:
                ...
                results_current_cutoff[EvaluatorMetrics.NDCG.value]                 += ndcg(recommended_items_current_cutoff, relevant_items, relevance=self.get_user_test_ratings(test_user))
