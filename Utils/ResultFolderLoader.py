...
def _get_printable_recommender_name(RECOMMENDER_NAME):
    ...
    recommender_printable_name = recommender_printable_name.replace("TopPopYearRange", "TopPop new releases")
...
def _get_algorithm_file_name_list(algorithm_list,
    ...
    for algorithm in algorithm_list:
        ...
        # if TopPopFeature, append the ICM_genres to the file name
        if algorithm in [TopPopFeature,]:
            algorithm_file_name = algorithm.RECOMMENDER_NAME + '_ICM_genres'

        # if TopPopYearRange, append the ICM_year to the file name
        if algorithm in [TopPopYearRange, ]:
            algorithm_file_name = algorithm.RECOMMENDER_NAME + '_ICM_year'
...
class ResultFolderLoader(object):
    ...
    def generate_latex_results(self, output_file_path,
        ...
        output_file_csv_path = output_file_path.replace(".txt", ".csv")
        result_dataframe.to_csv(output_file_csv_path)
